...
export type DeepMapStore<T extends BaseDeepMap> = {
  ...
  getKey: <K extends AllPaths<T> | "">(key: K) => FromPath<T, K>;
  ...
  /**
   * Change store value.
   * @param newValue — Update store value.
   */
  update: (value: Partial<T> | undefined) => void;
...
export function deepMap<T extends BaseDeepMap>(init?: T): DeepMapStore<T> {
  ...
  $deepmap.getKey = <K extends AllPaths<T> | "">(key: K): any => {
    return getPath(key, $deepmap.value as BaseDeepMap);
  };
  ...
  $deepmap.update = (value: Partial<T> | undefined): void => {
